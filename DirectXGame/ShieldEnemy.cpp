...
void ShieldEnemy::Initialize(Model* model, Camera* camera, const Vector3& position) {
	...
	if (shieldLRDirection_ == ShieldLRDirection::kLeft) {

		worldTransform_.rotation_.y = -3.0f * std::numbers::pi_v<float> / 4.0f;
	} else {
		worldTransform_.rotation_.y = 3.0f * std::numbers::pi_v<float> / 4.0f;
	}
}
...
void ShieldEnemy::BehaviorMoveUpdate() {
	...
	worldTransform_.rotation_.y = degree / 180.0f * std::numbers::pi_v<float> - 3.0f * std::numbers::pi_v<float> / 4.0f;
